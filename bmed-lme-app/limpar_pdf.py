...
def preencher_quadradinhos(dicionario, base_nome, valor, tamanho):
    """Auxiliar para distribuir caracteres em campos numerados (ex: cnes_1, cnes_2)"""
    if not valor: return
    # Remove qualquer máscara que possa ter vindo do Front-end
...
def gerar_lme_hospitalar(dados_extraidos):
    ...
    try:
        ...
        if "/AcroForm" not in reader.trailer["/Root"]:
            print("ERRO FATAL: O PDF lme_editavel.pdf não possui campos de formulário!")
            return None

        writer = PdfWriter()
        writer.append(reader)

        # 2. MAPEAMENTO DOS CAMPOS DE TEXTO LIVRE
        campos_final = {
            ...
            # --- AJUSTE DA INSTITUIÇÃO ---
            # ATENÇÃO: Se no seu PDF o campo de texto da instituição se chamar apenas 
            # "estabelecimento" ou "nome_estab", você deve alterar a chave abaixo para bater com o PDF.
            "estabelecimento_nome": dados_extraidos.get("estabelecimento_nome", ""),
        }

        # 3. MAPEAMENTO DOS QUADRADINHOS
        ...
        # --- AJUSTE DO CNES ---
        # Isso vai preencher as caixinhas nomeadas no PDF como: cnes_1, cnes_2, cnes_3... até cnes_7.
